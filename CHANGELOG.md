...
+ [新增] 内置 RH 渠道 Minimax-h3 八个应用的调用脚本，韭菜盒子渠道预置 `rh-aiapp｜应用名` 系列模型，选中应用即可按 `webappId` 生成视频。
+ [调整] 默认画幅由 1:1 改为 9:16（生图与生视频共用该设置）。
+ [调整] 本地已存过的配置里没改过的画幅与视频模式，会按新默认值自动迁移一次（1:1 → 9:16、首尾帧 → 全能参考）。
+ [调整] 视频模式默认改为「全能参考模式」。
+ [调整] 视频时长默认值由 6 秒改为 5 秒。
+ [调整] 视频时长按模型区分：MiniMax H3 系列（含 RH 应用）为 1–15 秒，其他视频模型仍为 4–30 秒。

## v0.19.2 - 2026-09-20

+ [新增] 配置里预置「韭菜盒子」渠道，接口地址与常用模型已填好，新用户只需填写自己的 API Key。
+ [新增] 配置里的韭菜盒子渠道新增「获取 Key」按钮，点击打开获取 API Key 的页面。

## v0.19.1 - 2026-09-20

+ [调整] 顶栏站点图标与浏览器图标替换为韭菜盒子 Logo，并按 Logo 原始比例调整顶栏图标占位。
+ [调整] Docker 镜像发布时同时打 `latest` 标签，自托管拉取 `latest` 也能拿到新版本。
+ [新增] 画布内置 MiniMax H3 与 Seedance 2.5 视频接口的调用脚本，模型名匹配且能力为视频时自动生效，不再需要逐个模型手动配置。
+ [新增] 新增「自定义域名部署」文档与 `deploy/nginx` 反向代理配置模板，说明如何用 Cloudflare 源站证书把画布挂到自有域名。
+ [新增] 新增「模型调用脚本示例」文档，给出 JSON 请求体视频接口的两份可直接复用的调用脚本。
+ [调整] Docker 镜像发布改用当前仓库所属命名空间，fork 后无需修改 workflow 即可发布自己的镜像。
+ [修复] Docker 部署文档更正主应用镜像实际由 nginx 托管静态前端产物，并补充自定义域名部署入口。
